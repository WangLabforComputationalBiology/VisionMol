        ...
        public override void ProcessInteractable(XRInteractionUpdateOrder.UpdatePhase updatePhase)
        {
            ...
            switch (updatePhase)
            {
                case XRInteractionUpdateOrder.UpdatePhase.Fixed:
                    ...
                    else if (anchorType == AnchorType.LeftPalmPosition)
                    {
                        if (HandTracking.LeftHandTrackInfo.IsValid)
                        {
                            transform.position = HandTracking.LeftHandTrackInfo.PalmPosition;
                            if (this.UpdateRotation)
                            {
                                transform.rotation = HandTracking.LeftHandTrackInfo.PalmRotation;
                            }
                        }
                    }
                    else if (anchorType == AnchorType.RightPalmPosition)
                    {
                        if (HandTracking.RightHandTrackInfo.IsValid)
                        {
                            transform.position = HandTracking.RightHandTrackInfo.PalmPosition;
                            if (this.UpdateRotation)
                            {
                                transform.rotation = HandTracking.RightHandTrackInfo.PalmRotation;
                            }
                        }
                    }
                    break;
